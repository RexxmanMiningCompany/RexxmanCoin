...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "QUARNetwork") type = QUARNetwork;
    if(strType == "QUARNetworkVariable") type = QUARNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == QUARNetwork) s = "QUARNetwork";
    if(type == QUARNetworkVariable) s = "QUARNetworkVariable";
